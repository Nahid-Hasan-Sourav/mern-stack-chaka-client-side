...
const AllSellers = () => {
  ...
  const { data: itemData = [], refetch } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        `https://a-12-chakka-server-side.vercel.app/all-seller`,
        {
          method: "GET",
          headers: {
  ...
  const handleVerifySeller = (id) => {
    ...
    fetch(`https://a-12-chakka-server-side.vercel.app/users/seller/${id}`, {
      method: "PUT",
      headers: {
        authorization: `bearer ${localStorage.getItem("chaka-token")}`,
      },
  ...
  const handleDelete = (id) => {
    console.log("id", id);
    fetch(
      `https://a-12-chakka-server-side.vercel.app/users/seller/deletes/${id}`,
      {
        method: "DELETE",
        headers: {
          authorization: `bearer ${localStorage.getItem("chaka-token")}`,
                  ...
                  <td>
                    <button>
                      {data?.sellerStatus ? (
                        ...
                        <span className="btn btn-xs btn-error">unverified</span>
                      )}
                    </button>
                  </td>
                  <td>
